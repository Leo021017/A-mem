        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.pp_enabled:
            ...
            for i in range(num_gpus):
                causal_mask = causal_mask.to(self.devices[i])
                for idx, layer in enumerate(self.layer_gpu_list[i]):
                    ...
                    hidden_states = layer(
                        ...
                        position_ids=(
                            position_ids.to(self.devices[i])
                            if position_ids is not None
                            else None
                        ),
                        past_key_values=past_key_values,
                        cache_position=(
                            cache_position.to(self.devices[i])
                            if cache_position is not None
                            else None
                        ),
                        position_embeddings=(
                            position_embeddings[0].to(self.devices[i]),
                            position_embeddings[1].to(self.devices[i]),
                        ),
                        **kwargs,
                    )
                start_idx += len(self.layer_gpu_list[i])
                if i != num_gpus - 1:
                    hidden_states = hidden_states.to(self.devices[i + 1])
                else:
                    hidden_states = hidden_states.to(self.devices[0])
        else:
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        if self.pp_enabled:
            ...
            # move layers to corresponding GPUs
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.pp_enabled:
            ...
            attention_mask = (
                attention_mask.to(self.devices[0])
                if attention_mask is not None
                else None
            )
            position_ids = (
                position_ids.to(self.devices[0]) if position_ids is not None else None
            )
            inputs_embeds = (
                inputs_embeds.to(self.devices[0]) if inputs_embeds is not None else None
            )
            cache_position = (
                cache_position.to(self.devices[0])
                if cache_position is not None
                else None
            )
        ...
        if self.pp_enabled:
            ...
            self.lm_head = self.lm_head.to(self.devices[-1])
